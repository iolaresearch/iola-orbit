...
def propagate_satellites():
    """
    Propagate all satellites in state.satrec_catalog to the current UTC time.

    Called on every GET /satellites request by api.py.
    Takes ~9ms for 15,000 satellites on the Render free tier.
    No cache. No background thread. Positions are exact for the moment
    of the request — zero staleness.

    Returns
    -------
    dict with keys:
      propagated_at  — ISO 8601 UTC string of this propagation epoch
      satellites     — list of satellite state dicts

    The result is returned directly to the API caller, never stored.
    """
    ...
    for entry in state.satrec_catalog:
    ...
    return {
        "propagated_at": current_utc.isoformat(),
